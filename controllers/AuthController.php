...
class Auth
{
    ...
    public function checkSession(){
        ...
        $query = mysqli_num_rows($this->db->query("SELECT * from logs where loginID='$this->loginID'"));

        if($query){
            $this->currentLog = mysqli_fetch_assoc($this->db->query("SELECT * from logs where loginID='$this->loginID', status='active' and loggedout=0"));
        ...
        if($loginQuery){
            ...
            if($loginQuery['password'] == md5($this->password)){
                ...
                if($insertLog){
                    ...
                    if(!empty($_GET['back'])) header("Location:".str_replace('$','&',$_GET['back']));
